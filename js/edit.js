...
function createRow(continut) {
    tableContent += getRow(continut);                // adaug in tableContent continutul rezultat din functia getRow
}

$.ajax('date/edit-list.php', {                            // aduce date din fisier continut2.json
    cache: false,
    dataType: 'json'
}).done(function (continut2) {                       // ajax face un request
    console.debug('continut loaded', continut2);     // stochez datele in variabila continut2
    continut2.forEach(createRow);                    // se creaza un string din variabila continut2, in care am stocat datele venite de pe server
    ...
    $('#question-list a.edit').click(function () {  // cu JS prind click. Caut titlul de tabel, sa fie link si clasa remove
        ...
        var continut = continut2.find(function(c) { // caut un continut dintre toate continut2le care contine id-ul meu
            return c.id == id;                    // se ia ID-ul
        })
        console.debug('remove', id, continut, this);
